export const translations = {
  AYO: {
    de: "Ayo!",
    en: "Ayo!",
    cs: "Čus!",
    pl: "Cześć!",
    sk: "Čau!",
  },
  BASIC_HEAD: {
    de: "Grundlegende Informationen",
    en: "Basic info",
    cs: "Základní informace",
    pl: "Podstawowe informacje",
    sk: "Základné informácie",
  },
  BROUGHT: {
    de: "Präsentiert von:",
    en: "Brought to you by:",
    cs: "Přináší vám:",
    pl: "Przedstawiają:",
    sk: "Prináša vám:",
  },
  DATE: {
    de: "10.-11. Juni",
    en: "10.-11. June",
    cs: "10.-11. Června",
    pl: "10.-11. Czerwca",
    sk: "10.-11. Júna",
  },
  DESIGNED: {
    de: "Entworfen und implementiert von",
    en: "Designed and implemented by",
    cs: "Návrh a implementace",
    pl: "Zaprojektowane i zaimplementowane przez",
    sk: "Návrh a implementácia",
  },
  DOCUMENTS: {
    de: "Dokumente",
    en: "Documents",
    cs: "Dokumenty",
    pl: "Dokumenty",
    sk: "Dokumenty",
  },
  FB_EV: {
    de: "fb ereignis",
    en: "fb event",
    cs: "fb událost",
    pl: "fb wydarzenie",
    sk: "fb udalosť",
  },
  FRIDAY: {
    de: "Freitag",
    en: "Friday",
    cs: "Pátek",
    pl: "Piątek",
    sk: "Piatok",
  },
  HERE: {
    de: "HIER",
    en: "HERE",
    cs: "ZDE",
    pl: "TUTAJ",
    sk: "TU",
  },
  HOW_MUCH: {
    de: "Wie viel?",
    en: "How much?",
    cs: "Kolik?",
    pl: "Ile?",
    sk: "Koľko?",
  },
  HOW_MUCH_METHOD: {
    de: "-nur online zahlungen werden akzeptiert!",
    en: "-only online payments are accepted!",
    cs: "-akceptujeme pouze online platby!",
    pl: "-akceptujemy tylko płatności online!",
    sk: "-akceptujeme iba online platby!",
  },
  HOW_MUCH_PRICE: {
    de: "-65€ für 2 Tage",
    en: "-65€ for 2 days",
    cs: "-65€ za 2 dny",
    pl: "-65€ za 2 dni",
    sk: "-65€ za 2 dni",
  },
  L_HOSTEL: {
    de: "Hostel",
    en: "Hostel",
    cs: "Hostel",
    pl: "Hostel",
    sk: "Hostel",
  },
  L_TRACK: {
    de: "Strecke",
    en: "Track",
    cs: "Trať",
    pl: "Tor",
    sk: "Trať",
  },
  LOC: {
    de: "Ort",
    en: "location",
    cs: "lokace",
    pl: "miejsce",
    sk: "miesto",
  },
  LOCATION: {
    de: "Čadca, Slowakei",
    en: "Čadca, Slovakia",
    cs: "Čadca, Slovensko",
    pl: "Čadca, Słowacja",
    sk: "Čadca, Slovensko",
  },
  LOCATIOS: {
    de: "ORTE",
    en: "LOCATIONS",
    cs: "LOKACE",
    pl: "MIEJSCA",
    sk: "MIESTA",
  },
  MEDIA: {
    de: "Medien",
    en: "Media",
    cs: "Média",
    pl: "Media",
    sk: "Média",
  },
  MEDIA_DESC: {
    de: "Schau dir einige Aufnahmen von ADHS an, die den Spot jedes Jahr shredden!",
    en: "Check out some shots from ADHS who shred the place annually!",
    cs: "Podívejte se na nějaké záběry od ADHS, kteří spot shredí každý rok!",
    pl: "Zobacz kilka ujęć od ADHS, którzy rokrocznie shredują to miejsce!",
    sk: "Pozrite si nejaké zábery od ADHS, ktorí spot shredia každý rok!",
  },

  MORE_INFO: {
    de: "Weitere Informationen werden hinzugefügt...",
    en: "More info will be added...",
    cs: "Více informací přidáme brzy...",
    pl: "Więcej informacji zostanie dodanych wkrótce...",
    sk: "Viac informácií pridáme čoskoro...",
  },
  P_ARRIVE: {
    de: "Ankunft im Hostel",
    en: "Bus arrives at the hostel",
    cs: "Autobus přijíždí do hostelu",
    pl: "Autobus przyjeżdża do hostelu",
    sk: "Autobus prichádza do hostela",
  },
  P_FIRST: {
    de: "Erste Fahrt",
    en: "First ride",
    cs: "První jízda",
    pl: "Pierwsza jazda",
    sk: "Prvá jazda",
  },
  P_LAST: {
    de: "Letzte Fahrt",
    en: "Last ride",
    cs: "Poslední jízda",
    pl: "Ostatnia jazda",
    sk: "Posledná jazda",
  },
  P_LEAVE: {
    de: "Bus fährt zur Strecke",
    en: "Bus leaves for the track",
    cs: "Autobus odjíždí na trať",
    pl: "Autobus odjeżdża na tor",
    sk: "Autobus odchádza na trať",
  },
  P_REGO: {
    de: "Registrierung im Hostel",
    en: "Registration at the hostel",
    cs: "Registrace v hostelu",
    pl: "Rejestracja w hostelu",
    sk: "Registrácia v hosteli",
  },
  P_START: {
    de: "Beginn des Events",
    en: "Beginning of the event",
    cs: "Začátek akce",
    pl: "Początek wydarzenia",
    sk: "Začiatok akcie",
  },

  PROGRAM: {
    de: "Programm",
    en: "Program",
    cs: "Program",
    pl: "Program",
    sk: "Program",
  },
  RACE: {
    de: "Rennen?",
    en: "Racing?",
    cs: "Závody?",
    pl: "Wyścigi?",
    sk: "Závody?",
  },
  RACE_PRIZE: {
    de: "-Preis: beginnt bei 250€ CA$H",
    en: "-prize: pool begins at 250€ CA$H",
    cs: "-cena: začíná na 250€ CA$H",
    pl: "-nagroda: pula zaczyna się od 250€ CA$H",
    sk: "-cena: začína na 250€ CA$H",
  },
  RACE_TYPE: {
    de: "-no paws down wenn genug Leute dafür sind",
    en: "-no paws down if enough people are up for it",
    cs: "-no paws down pokud bude dostatek lidí",
    pl: "-no paws down jeśli wystarczy ludzi",
    sk: "-no paws down ak bude dostatok ľudí",
  },
  REGISTRATION: {
    de: "Registrierung",
    en: "Registration",
    cs: "Registrace",
    pl: "Rejestracja",
    sk: "Registrácia",
  },
  REGO: {
    de: "Wann Registrierung?",
    en: "When registration?",
    cs: "Kdy registrace?",
    pl: "Kiedy rejestracja?",
    sk: "Kedy registrácia?",
  },
  REGO_NOW: {
    de: "JETZT!",
    en: "NOW!",
    cs: "TEĎ!",
    pl: "TERAZ!",
    sk: "TERAZ!",
  },
  SATURDAY: {
    de: "Samstag",
    en: "Saturday",
    cs: "Sobota",
    pl: "Sobota",
    sk: "Sobota",
  },
  SUNDAY: {
    de: "Sonntag",
    en: "Sunday",
    cs: "Neděle",
    pl: "Niedziela",
    sk: "Nedeľa",
  },
  SPONSORS: {
    de: "Sponsoren",
    en: "Sponsors",
    cs: "Sponzoři",
    pl: "Sponsorzy",
    sk: "Sponzori",
  },
  TITLE_1: {
    de: "Kurz",
    en: "Short",
    cs: "Krátké",
    pl: "Krótkie",
    sk: "Krátke",
  },
  TITLE_2: {
    de: "Steil",
    en: "Steep",
    cs: "Strmé",
    pl: "Strome",
    sk: "Strmé",
  },
  TITLE_3: {
    de: "Schnell",
    en: "Fast",
    cs: "Rychlé",
    pl: "Szybkie",
    sk: "Rýchle",
  },
  TRACK: {
    de: "Strecke",
    en: "Track",
    cs: "Trať",
    pl: "Tor",
    sk: "Trať",
  },
  TRACK_DESC: {
    de: "Die Strecke ist zwar kurz aber man kann auf ihr hohe Geschwindigkeiten erreichen. Falls Sie auf beiden Seiten sliden können, sollte das für Sie jedoch kein Problem sein.",
    en: "The track is short but lets you get quite some speed if you want. But you shouldn't worry if you can sldie both ways.",
    cs: "Trať je krátká, ale docela rychlá, pokud chcete. Nemusíte se ale bát, pokud umíte slide na obě strany.",
    pl: "Tor jest krótki, ale pozwala na osiągnięcie dość dużej prędkości, jeśli chcesz. Ale nie powinieneś się martwić, jeśli możesz slie obiema stronami.",
    sk: "Trať je krátka, ale celkom rýchla, ak chcete. Nemusíte sa ale báť, ak viete slide na obe strany.",
  },
  TRACK_DESC_2: {
    de: "Für alle, die Ihre Fähigkeiten noch verbessern möchten, bieten wir wärend des gesammten Wochenendes kostenlose Slide-Kurse an.",
    en: "For anyone who wants to improve their skills, we will be offering free slide lessons for the whole duration of the event.",
    cs: "Pro každého, kdo si chce zlepšit své dovednosti, budeme po celou dobu akce nabízet bezplatné lekce slidu.",
    pl: "Dla wszystkich, którzy chcą poprawić swoje umiejętności, będziemy oferować bezpłatne lekcje slajdów przez cały czas trwania wydarzenia.",
    sk: "Pre každého, kto si chce zlepšiť svoje skills, budeme po celú dobu akcie ponúkať bezplatné lekcie slidu.",
  },
  TRACK_DIFFICULTY: {
    de: "Schwierigkeit:",
    en: "difficulty:",
    cs: "obtížnost:",
    pl: "trudność:",
    sk: "obtiažnosť:",
  },
  TRACK_GRADE: {
    de: "Steigung: bis zu 18%",
    en: "grade: up to 18%",
    cs: "sklon: až 18%",
    pl: "stopień: do 18%",
    sk: "sklon: až 18%",
  },
  TRACK_LENGTH: {
    de: "Länge: ~600m",
    en: "length: ~600m",
    cs: "délka: ~600m",
    pl: "długość: ~600m",
    sk: "dĺžka: ~600m",
  },
  WHEN: {
    de: "Wann?",
    en: "When?",
    cs: "Kdy?",
    pl: "Kiedy?",
    sk: "Kedy?",
  },
  WHEN_DATE: {
    de: "-10. - 11. Juni 2023",
    en: "-10th - 11th June 2023",
    cs: "-10. - 11. Června 2023",
    pl: "-10. - 11. Czerwca 2023",
    sk: "-10. - 11. Júna 2023",
  },
  WHEN_DAYS: {
    de: "-zwei Tage fahren",
    en: "-two days of riding",
    cs: "-dva dny ježdění",
    pl: "-dwa dni jazdy",
    sk: "-dva dni jazdenia",
  },
  WHERE: {
    de: "Wo?",
    en: "Where?",
    cs: "Kde?",
    pl: "Gdzie?",
    sk: "Kde?",
  },
  DESCRIPTION: {
    de: `Für unsere historisch erste eigene Veranstaltung
    haben wir unsere Lieblingsstrecke in der Slowakei ausgewählt.
    <br />
    Die Kysuce Scythe Session ist eine kurze, aber sehr
    steile Strecke im Norden der Slowakei.
    <br />
    Die Strecke wird nur für uns vollständig abgesperrt, die Ecken mit Heuballen gesichert und ein 
    <span>SHUTTLE-VAN</span> bereitgestellt. Der schöne glatte Asphalt
    lockt mit einer Steigung von bis zu <span>18%</span>💀
    <br />
    Während des Events können Sie unsere neuesten Reaper Trucks
    und andere zukünftige Produkte der SCYTHE Küche.
    <br />
    Falls Sie kompetitiv sind, haben wir gute Neuigkeiten für Sie:
    Teil des Events wird ein kurzes No-Paws-Down-Rennen sein.
    <br />
    Über die Nacht können Sie auf dem örtlichen Parkplatz oder mit einer
    Hängematte im umliegenden Wald campen. Während des ganzen Events steht
    für jeden Teilnehmer kostenloses Wasser zur Verfügung
    <br />
    Wir freuen uns auf Sie!
    <br />
    Cheers!
    <br />
    SCYTHE
    `,
    en: `For the first time ever we have decided to hold an event on one
    of our favorite spots in Slovakia.
    <br />
    Kysuce Scythe Session takes place on a short but steep track in
    the north of Slovakia.
    <br />
    The track will be completely closed with a
    <span>VAN SHUTTLE</span>, hay in the corners and smooth, hot
    pavement with grade up to <span>18%</span>💀.
    <br />
    Also, you'll be able to try out our newest Reaper trucks and
    whatever else we are cooking for the future of Scythe.
    <br />
    For those who are up for some <span>racing</span> action, we
    will also hold a quick no-paws-down style race.
    <br />
    We will be also providing a space for camping on a nearby
    parking lot, or you can hang your hammock in the trees. Water
    will also be available on site for every rider.
    <br />
    We hope to see you soon.
    <br />
    Cheers!
    <br />
    SCYTHE`,
    cs: `Poprvé v historii jsme se rozhodli uspořádat náš vlastní event a 
    zvolili jsme naše oblíbené místo na Slovensku.
    <br />
    Kysuce Scythe Session je sice krátká, ale zato velmi prudká trať na severu Slovenska.
    <br />
    Trať bude kompletně uzavřená jen pro nás, s 
    <span>VÝVOZY DODÁVKOU</span>, balíkama v zatáčkách a nádherně 
    hladkou cestou se sklonem až <span>18%</span>💀.
    <br />
    Budete si moct vyzkoušet naše nejnovější trucky a další produkty, které připravujeme.
    <br />
    Pokud budete chtít závodit, máme pro vás dobrou zprávu, 
    můžete se zúčastnit krátkého no-paws-down závodu!
    <br />
    Můžete se utábořit na zdejším parkovišti, nebo pokud máte hamaku,
    tak v okolním lese. Voda zdarma je dostupná pro každého účastníka.
    <br />
    Vidíme se brzy!
    <br />
    Zdar!
    <br />
    SCYTHE`,
    pl: `Po raz pierwszy w historii postanowiliśmy zorganizować własną imprezę
    i wybraliśmy nasze ulubione miejsce na Słowacji.
    <br />
    Kysuce Scythe Session to krótka, ale bardzo stroma trasa na północy Słowacji.
    <br />
    Tor będzie całkowicie zamknięty tylko dla nas, z 
    <span>TRANSPORTEM NA GÓRĘ</span>, belami w zakrętach
    i idealne gładką drogą o nachyleniu do <span>18%</span>💀.
    <br />
    Będziecie mogli wypróbować nasze najnowsze ciężarówki
    i inne produkty, które przygotowujemy.
    <br />
    Jeśli chcesz się ścigać, mamy dla Ciebie dobrą wiadomość,
    możesz wziąć udział w krótkim wyścigu bez łap w dół!
    <br />
    Biwakować można na miejscowym parkingu lub, jeśli macie hamak, w okolicznym lesie.
    Dla każdego uczestnika dostępna jest bezpłatna woda.
    <br />
    Do zobaczenia wkrótce!
    <br />
    Powodzenie!
    <br />
    SCYTHE`,
    sk: `Po prvý krát v histórii sme sa rozhodli usporiadať náš vlastný event a
    rozhodli pre naše obľúbené miesto na Slovensku.
    <br />
    Kysuce Scythe Session je síce krátka, ale zato veľmi prudká trať na severe Slovenska.
    <br />
    Trať bude kompletné uzavretá len pre nás, s
    <span>VÝVOZMI DODÁVKOU</span>, balíkami v zákrutách a nádhernou
    hladkou cestou so sklonom až <span>18%</span>💀.
    <br />
    Budete si môcť vyskúšať naše najnovšie trucky a ďalšie produkty, ktoré pripravujeme.
    <br />
    A ak budete mať chuť závodiť, máme pre vás dobrú správu,
    môžete sa zúčastniť krátkeho no-paws-down závodu!
    <br />
    Môžete sa utáboriť na miestnom parkovisku, alebo ak máte hamaku,
    tak v okolitom lese. Voda zadarmo je dostupná pre každého zúčastneného.
    <br />
    Vidíme sa čoskoro!
    <br />
    Zdar!
    <br />
    SCYTHE`,
  },
};
